    ...
    User.find({ group: groupId })
        .then((usersFromGroup) => {
            ...
            for(let el of usersFromGroup){
                Event.find({ organizer: el._id, createdAt : {$gte : req.payload.lastReadNotif } })
                .then((newEvents)=> newNotifs.push(newEvents))
                .catch(err => console.log("there was an error looking for new Events", err))
                Project.find({ creator: el._id, createdAt : {$gte : req.payload.lastReadNotif } })
                .then((newProjects)=> newNotifs.push(newProjects))
                .catch(err => console.log("there was an error looking for new Projects", err)) 
            }
        })
        .then(() => {
